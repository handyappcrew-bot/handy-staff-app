...
const buildHistory = (
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    if (isHoliday) {
      items.push({ date: `${month}월 ${day}일`, dayOfWeek, calendarDate: day, isHoliday: true, badges: [{ label: "휴무", bg: '#DBDCDF', color: '#93989E' }], startTime: "00:00", endTime: "00:00", sheetStatus: "휴무", isClosed: false, breakMinutes: 0, overtimeMinutes: 0 });
    } else if (d) {
...
const AttendanceManagement = () => {
  ...
  return (
    <div className="mx-auto max-w-lg min-h-screen" style={{ backgroundColor: '#FFFFFF' }}>
      <div style={{ paddingBottom: 'calc(env(safe-area-inset-bottom) + 80px)', backgroundColor: '#FFFFFF' }}>
                  ...
                  {week.map((day, di) => {
                    ...
                    return (
                      <div key={di} className="pressable flex flex-col items-center py-1.5 min-h-[90px] cursor-pointer" onClick={() => openSheetByDay(day)}>
                        <div style={{ height: '22px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '4px' }}>
                          <span style={{ fontSize: '14px', fontWeight: 500, letterSpacing: '-0.02em', color: isToday ? '#FFFFFF' : isHoliday ? '#DBDCDF' : isSunday ? '#FF5959' : isSaturday ? '#5DB1FF' : '#70737B', ...(isToday ? { backgroundColor: '#4261FF', borderRadius: '10px', minWidth: '40px', width: '40px', height: '22px', display: 'flex', alignItems: 'center', justifyContent: 'center' } : {}), ...(isHoliday ? { textDecoration: 'line-through', textDecorationColor: '#DBDCDF' } : {}) }}>{day}</span>
                        </div>
