  ...
  return (
    <div className="min-h-screen max-w-lg mx-auto" style={{ backgroundColor: '#FFFFFF' }}>
      ...
      <div className="flex items-center gap-2 px-2 pt-4 pb-2 sticky top-0 z-10" style={{ backgroundColor: '#FFFFFF' }}>
        <button onClick={() => navigate("/")} className="pressable p-1"><ChevronLeft className="w-6 h-6 text-foreground" /></button>
      ...
      {activeTab === "submit" ? (
        <div className="px-5 pt-5 pb-8" style={{ backgroundColor: '#FFFFFF' }}>
          ...
          <div className="flex gap-3 mt-4 overflow-x-auto scrollbar-none">
            <button onClick={() => images.length < 2 && setShowUploadSheet(true)} disabled={images.length >= 2}
              className={`pressable w-[100px] h-[100px] rounded-2xl flex flex-col items-center justify-center flex-shrink-0 transition-colors ${images.length >= 2 ? "bg-[hsl(0,0%,85%)]" : "bg-primary/10"}`}>
          ...
          <button disabled={!isFormValid} onClick={() => setShowSubmitDialog(true)}
            className={`pressable w-full py-4 rounded-xl text-base font-semibold mt-4 transition-colors ${isFormValid ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>건의하기</button>
        </div>
      ) : (
        <div className="px-5 pt-4 pb-8" style={{ backgroundColor: '#FFFFFF' }}>
