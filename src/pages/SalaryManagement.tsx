    ...
    <div className="mx-auto max-w-lg min-h-screen" style={{ backgroundColor: '#FFFFFF' }}>
      ...
      {activeTab === "calendar" ? (
        <div className="pb-8">
            ...
            <div style={{ padding: '16px 16px 0 16px' }}>
              ...
              <div className="flex items-center justify-between" style={{ marginTop: '8px' }}>
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => setSalaryInfoOpen(true)} className="pressable">
          ...
          <div className="w-full h-[12px]" style={{ backgroundColor: '#F7F7F8', marginTop: '20px' }} />
          ...
          <div className="flex items-center justify-between px-5 py-4">
            <div className="flex items-center gap-1">
              ...
              <button onClick={goNextMonth} className="pressable p-1" disabled={isCurrentMonth}>
                <ChevronRight className="h-5 w-5" style={{ color: isCurrentMonth ? '#D1D5DB' : '#19191B' }} />
              </button>
            </div>
            <div className="flex">
              <button onClick={() => setViewMode("monthly")} className="pressable" style={{ width: '36px', height: '22px', borderRadius: '4px 0 0 4px', backgroundColor: viewMode === "monthly" ? '#93989E' : '#F7F7F8', fontSize: '12px', fontWeight: 600, letterSpacing: '-0.02em', color: viewMode === "monthly" ? '#FFFFFF' : '#93989E' }}>월간</button>
              <button onClick={() => setViewMode("daily")} className="pressable" style={{ width: '36px', height: '22px', borderRadius: '0 4px 4px 0', backgroundColor: viewMode === "daily" ? '#93989E' : '#F7F7F8', fontSize: '12px', fontWeight: 600, letterSpacing: '-0.02em', color: viewMode === "daily" ? '#FFFFFF' : '#93989E' }}>일간</button>
            </div>
          </div>
                      ...
                      return (
                        <div key={di} className="flex flex-col items-center py-1.5" style={{ minHeight: '72px' }}>
                          ...
                          {day.workPay && !day.isOutside && (
                            <button onClick={() => handleDayClick(day)} className="pressable" style={{ width: '40px', height: '17px', borderRadius: '4px', backgroundColor: '#F7F7F8', fontSize: '12px', fontWeight: 500, letterSpacing: '-0.02em', color: '#AAB4BF', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginBottom: day.weeklyHolidayPay ? '4px' : '0' }}>
                              {day.workPay}만
                            </button>
                          )}
                          {day.weeklyHolidayPay && !day.isOutside && (
                            <button onClick={() => handleDayClick(day)} className="pressable" style={{ width: '40px', height: '17px', borderRadius: '4px', backgroundColor: '#E8F3FF', fontSize: '12px', fontWeight: 500, letterSpacing: '-0.02em', color: '#7488FE', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                              {day.weeklyHolidayPay}만
                            </button>
                          )}
                          {day.isPayday && !day.isOutside && (
                            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" style={{ marginTop: day.workPay ? '2px' : '0', flexShrink: 0 }}>
                              <circle cx="8" cy="8" r="7.5" fill="#FFB300" stroke="#F59E0B" strokeWidth="0.5"/>
        ...
        <div className="pb-8" style={{ backgroundColor: '#FFFFFF' }}>
          ...
          <div className="space-y-3 px-5">
            {MOCK_PAY_STUBS.map((stub) => (
              <button key={stub.id} onClick={() => navigate(`/salary/pay-stub/${stub.id}`)} className="pressable w-full rounded-2xl bg-white p-5 text-left" style={{ boxShadow: '2px 2px 12px rgba(0,0,0,0.06)' }}>
      ...
      {/* 급여 상세 바텀시트 */}
      <Sheet open={bottomSheetOpen && !!selectedDay?.detail} onOpenChange={(v) => { if (!v) setBottomSheetOpen(false); }}>
