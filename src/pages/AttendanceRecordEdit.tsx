...
const ReasonInputSheet = ({ open, value, onClose, onConfirm }: ReasonInputSheetProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50" onClick={onClose}>
      <div
        className="w-full max-w-lg rounded-t-3xl bg-card px-6 pt-6 shadow-xl animate-in slide-in-from-bottom"
        style={{ paddingBottom: `${keyboardHeight + 32}px` }}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-3">
          <button
            onClick={onClose}
            className="pressable flex-1 h-12 rounded-xl bg-[hsl(var(--secondary))] text-primary font-semibold text-[15px]"
          >
            취소
          </button>
          <button
            ...
            className={`pressable flex-1 h-12 rounded-xl font-semibold text-[15px] ${
              text.trim()
...
const AttendanceRecordEdit = () => {
  ...
  const STATUS_LABEL: Partial<Record<string, { bg: string; color: string; label: string }>> = {
    "휴무": { bg: '#DBDCDF', color: '#93989E', label: '휴무' },
  ...
  const ShiftBadges = () => {
    if (isUnregistered) {
      return <span style={{ borderRadius: '4px', padding: '4px 8px', fontSize: '13px', fontWeight: 500, backgroundColor: '#F7F7F8', color: '#AAB4BF' }}>무일정</span>;
    }
    const statusBadge = STATUS_LABEL[detail?.status ?? ""];
    if (statusBadge) {
      return <span style={{ borderRadius: '4px', padding: '4px 8px', fontSize: '13px', fontWeight: 500, backgroundColor: statusBadge.bg, color: statusBadge.color }}>{statusBadge.label}</span>;
    }
      ...
      <div className="px-5 pt-6 pb-8">
        {step === "reason" ? (
          <>
            ...
            <div className="rounded-xl px-4 py-3 flex items-center gap-2 mb-8 flex-wrap" style={{ backgroundColor: '#F0F7FF' }}>
              <ShiftBadges />
              <span className="text-[15px] font-semibold text-foreground">{dateLabel}</span>
              {!isUnregistered && detail?.startTime && detail.startTime !== "00:00" && (
                <span style={{ fontSize: '15px', fontWeight: 400, color: '#93989E', letterSpacing: '-0.02em' }}>{detail.startTime} - {detail.endTime}</span>
              )}
            </div>
          ...
          <>
            ...
            <button onClick={() => setMinutePickerOpen(true)} className="pressable w-full flex items-center justify-between border border-border rounded-xl px-4 py-3.5 mb-6 text-left">
              <span className="text-[15px] font-medium" style={{ color: '#FF862D' }}>{breakMinutes}</span>
              <span className="text-[15px] text-muted-foreground">분</span>
            </button>
          ...
          <>
            ...
            <button onClick={() => openTimePicker("end")} className="pressable w-full flex items-center justify-between border border-border rounded-xl px-4 py-3.5 mb-6 text-left">
              <span className="text-[15px]" style={{ color: '#FF862D' }}>{formatAmPm(endTime)}</span>
              <ChevronDown className="h-5 w-5 text-muted-foreground" />
            </button>
      ...
      <div className="px-5 pb-8 pt-4">
        {step === "reason" ? (
          <button
            ...
            onClick={handleNext}
            className={`pressable w-full h-14 rounded-2xl text-lg font-semibold ${selectedReason ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground cursor-not-allowed"}`}
          >
            다음
          </button>
        ) : (
          <button
            ...
            className={`pressable w-full h-14 rounded-2xl text-lg font-semibold ${canSubmit ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground cursor-not-allowed"}`}
          >
            수정 요청하기
          </button>
        )}
