...
export default function BoardWrite() {
  ...
  return (
    <div className="min-h-screen bg-white max-w-lg mx-auto">
      ...
      {/* 콘텐츠 영역 — 하단 fixed 영역(사진+버튼 약 220px) + BottomNav(74px) 여백 확보 */}
      <div className="px-5 pt-4 pb-[260px]">
        {/* 카테고리 선택 */}
        ...
        <input type="text" placeholder="제목을 입력해주세요 (최대 20자)" maxLength={20}
          ...
          data-no-focus="true" className="w-full border-b border-border pb-3 mb-3 bg-transparent outline-none"
        ...
        <textarea placeholder="내용을 입력하세요. (최대 1000자)" maxLength={1000}
          ...
          data-no-focus="true" className="w-full bg-transparent resize-none outline-none"
          style={{ fontSize: '14px', letterSpacing: '-0.02em', color: '#19191B', minHeight: '300px', display: 'block' }} />
      </div>

      {/* 하단: 사진 + 등록 버튼 */}
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-lg z-30 border-t border-border bg-white px-5 pt-3 pb-8">
        <div className="flex gap-3 overflow-x-auto pb-4" style={{ scrollbarWidth: 'none' }}>
          ...
          <button
            ...
            className="pressable flex-shrink-0 flex flex-col items-center justify-center gap-1 rounded-xl border border-border"
        ...
        <button onClick={() => canSubmit && setRegisterDialog(true)} disabled={!canSubmit}
          className="pressable w-full rounded-2xl py-4 font-semibold"
      ...
      <ConfirmDialog open={exitDialog} onOpenChange={setExitDialog}
        title={isEdit ? "게시글 수정 취소" : "게시글 작성 취소"}
        description={isEdit ? <>게시글 수정을 취소하시겠어요?<br />변경된 내용은 저장되지 않아요</> : <>게시글 작성을 취소하시겠어요?<br />작성 중인 내용은 저장되지 않아요</>}
        buttons={[{ label: "계속 작성", onClick: () => setExitDialog(false), variant: "cancel" }, { label: "취소하기", onClick: () => navigate(-1) }]} />
