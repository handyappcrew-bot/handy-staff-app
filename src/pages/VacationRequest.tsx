    ...
    <div className="mx-auto min-h-screen max-w-lg bg-white">
      ...
      {step === "select-dates" && (
        <div className="">
            ...
            {weeks.map((week, wi) => (
              <div key={wi} className="grid grid-cols-7 mb-1">
                {week.map((d, di) => {
                  ...
                  const isSun = di === 0;
                  const isSat = di === 6;
                  const dateColor = d.isOutside ? '#AAB4BF' : isSelected ? '#FFFFFF' : isTodayDate ? '#FFFFFF' : isHoliday ? '#DBDCDF' : isSun ? '#FF5959' : isSat ? '#5DB1FF' : '#70737B';
                  return (
                    <button key={di} onClick={() => { if (!d.isOutside) toggleDate(key); }} className="pressable flex flex-col items-center py-1.5 w-full" style={{ minHeight: '90px' }} disabled={d.isOutside}>
                      <div style={{ height: '22px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '4px' }}>
                        <span style={{ fontSize: '14px', fontWeight: 500, letterSpacing: '-0.02em', color: dateColor, ...(isSelected ? { backgroundColor: '#4261FF', borderRadius: '10px', minWidth: '40px', width: '40px', height: '22px', display: 'flex', alignItems: 'center', justifyContent: 'center' } : isTodayDate ? { backgroundColor: '#4261FF', borderRadius: '10px', minWidth: '40px', width: '40px', height: '22px', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0.5 } : {}), ...(isHoliday ? { textDecoration: 'line-through', textDecorationColor: '#DBDCDF' } : {}) }}>
                          {d.date}
                        </span>
                      </div>
          ...
          <div className="px-5 pb-8 pt-8">
            <button disabled={selectedDates.length === 0} onClick={() => setStep("form")} className="pressable w-full rounded-2xl py-4 text-[16px] font-semibold" style={{ backgroundColor: selectedDates.length > 0 ? '#4261FF' : '#E5E7EB', color: selectedDates.length > 0 ? '#FFFFFF' : '#9CA3AF' }}>다음</button>
          </div>
        </div>
      )}
      ...
      {step === "form" && (
        <div className="">
              ...
              {sortedSelectedDates.map(dateKey => {
                ...
                return (
                  <div key={dateKey} className="flex items-center gap-3 rounded-xl bg-muted px-4 py-3">
                    <span style={{ borderRadius: '4px', padding: '4px 8px', fontSize: '13px', fontWeight: 500, backgroundColor: typeStyle[schedule.type].bg, color: typeStyle[schedule.type].text }}>{typeLabels[schedule.type]}</span>
          ...
          <div className="px-5 pb-8 pt-8">
            <button disabled={!isFormValid} onClick={() => setConfirmDialogOpen(true)} className="pressable w-full rounded-2xl py-4 text-[16px] font-semibold" style={{ backgroundColor: isFormValid ? '#4261FF' : '#E5E7EB', color: isFormValid ? '#FFFFFF' : '#9CA3AF' }}>휴가 요청하기</button>
          </div>
        </div>
      )}
