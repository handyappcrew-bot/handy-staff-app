...
const AttendanceCard = ({
  ...
  // Status badge
  const getBadge = () => {
    const style = { minWidth: 'auto', height: '28px', marginLeft: '0px', marginTop: '0px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', borderRadius: '9999px', fontSize: '14px', fontWeight: 600, letterSpacing: '-0.01em', padding: '0 10px' };
    switch (status) {
      case "holiday":
      ...
      case "late":
        return <span style={{ ...style, backgroundColor: '#FFEEE2', color: '#FF862D' }}>지각</span>;
      case "absent":
  ...
  const leftTimeColor = (status === "holiday" || status === "before_work") ? "text-muted-foreground" : status === "late" ? "" : status === "absent" ? "text-status-absent" : (status === "overtime" || status === "off_work") ? "text-primary" : "text-status-green";
  const leftTimeStyle = status === "late" ? { color: '#FF862D' } : {};
  ...
  return (
    <div className="mx-5 rounded-2xl bg-card p-5" style={{ boxShadow: '2px 2px 12px rgba(0,0,0,0.06)' }}>
      ...
      <div className="mb-1.5 flex items-center justify-between text-sm">
        <span className={leftTimeColor} style={leftTimeStyle}>{leftTimeLabel}</span>
        <span style={{
          width: '64px',
          height: '20px',
          borderRadius: '16px',
        ...
        {workBeforeBreakWidth > 0 && (
          <div
            ...
            style={{
              ...
              backgroundColor: status === "absent" ? "hsl(var(--status-absent))" : status === "late" ? "#FF862D" : (status === "overtime" || status === "off_work") ? "hsl(var(--primary))" : "hsl(var(--progress-green))",
            }}
          />
        )}
