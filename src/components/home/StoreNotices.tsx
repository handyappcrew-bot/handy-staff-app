  ...
  return (
    <div className="px-5">
      <div className="mb-3 flex items-end justify-between">
        ...
        <button onClick={() => navigateTo("/board", "게시판으로 이동했어요")} className="pressable flex items-center text-muted-foreground mb-0.5" style={{ fontSize: '14px' }}>
          더보기 <ChevronRight className="h-3.5 w-3.5" />
        </button>
      </div>
      <div className="flex flex-col gap-3">
        {notices.map((notice) => (
          <div key={notice.id} className="pressable flex items-center gap-3 rounded-xl bg-card p-4 cursor-pointer" style={{ boxShadow: '2px 2px 12px rgba(0,0,0,0.06)' }} onClick={() => navigateTo(`/board/${notice.id}`, "게시판으로 이동했어요")}>            <div className="flex h-[50px] w-[50px] shrink-0 items-center justify-center rounded-full bg-muted">
