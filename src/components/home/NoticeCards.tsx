...
const NoticeCards = ({ notices, onDismiss }: NoticeCardsProps) => {
  ...
  const handleClick = (notice: NoticeCard) => {
    // 카드 클릭 시 자동 dismiss
    onDismiss(notice.id);
    if (notice.type === "board") navigateTo(`/board/${notice.id}`, "게시판으로 이동했어요", { showLoading: true, forceAll: true });
    else if (notice.type === "salary") navigateTo("/salary/pay-stub/1", "급여 명세서로 이동했어요", { showLoading: true, forceAll: true });
  };
